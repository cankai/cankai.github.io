...
bool ConfigReader::load()
{
	...
	{
		LOG4CPLUS_ERROR(logger, "Configuration file name not specified!");
		return false;
	}
	LOG4CPLUS_DEBUG(logger, "Configuration file: "<<m_file_name);
	...
	if(!fp)
	{
		LOG4CPLUS_ERROR(logger, "Open configure file: "<<m_file_name<<" failed!");
		return false;
	}
...
string ConfigReader::get_string(const char* key, const char* default_value, bool log_warning)
{
	...
	if(target != NULL)
	{
		LOG4CPLUS_DEBUG(logger, "found config item: " << key << "=" << target->m_value);
	...
	else
	{
		if(log_warning)
		{
			LOG4CPLUS_WARN(logger, "Config item '" << key << "' not found in file [" << m_file_name << "].");
		}
		return default_value ? default_value : "";	
	}
...
ConfigWriter::ConfigWriter(const char* file_name) : ConfigReader(file_name)
{
	...
	if(!succ)
	{
		LOG4CPLUS_ERROR(logger, "failed to load configuration file '" << file_name << "'!");
	}
}
...
bool ConfigWriter::save(const char * file_name)
{
	...
	if(filepath == NULL)
	{
		LOG4CPLUS_ERROR(logger, "config file name not specified when try to save it!");
		return false;
	}
	...
	if(fp == NULL)
	{
		LOG4CPLUS_ERROR(logger, "can not open config file '" << filepath << "' in write mode!");
		return false;
	}
